...
//        ContextUtils cu = new ContextUtils(_textViewGroupName.getContext());
        ...
        _textViewGroupName.setTextColor(/*cu.shouldColorOnTopBeLight(Setup.appSettings().getDesktopFolderColor())*/false ? Color.WHITE : Color.BLACK);
...
//        cu.freeContextRef();
        ...
        int iconSize = Tool.dp2px(/*Setup.appSettings().getDesktopIconSize()*/52);
        ...
        for (int x2 = 0; x2 < cellSize[0]; x2++) {
            for (int y2 = 0; y2 < cellSize[1]; y2++) {
                ...
                final App app = MainActivity.Companion.getAppLoader().findItemApp(groupItem);
                if (app == null || /*AppSettings.get().getHiddenAppsList().contains(app.getComponentName())*/false) {
                    ...
                    view.setOnLongClickListener(new View.OnLongClickListener() {
                        ...
                        public boolean onLongClick(View view2) {
                            if (/*Setup.appSettings().getDesktopLock()*/false) return false;
        ...
        _cx = popupWidth / 2;
        _cy = popupHeight / 2 - (/*Setup.appSettings().getDesktopShowLabel()*/false ? Tool.dp2px(10) : 0);
        ...
        if (item._location == Definitions.ItemPosition.Dock) {
            ...
            _cy += iconSize / 2 + (/*Setup.appSettings().getDockShowLabel()*/false ? 0 : Tool.dp2px(10));
        }
    ...
    private void expand() {
        ...
        int startRadius = Tool.dp2px(52 / 2);

        long animDuration = 80 * 10;
    ...
    private void removeItem(Context context, final Item currentItem, Item dragOutItem, AppItemView currentView) {
        ...
        MainActivity.Companion.getDb().saveItem(dragOutItem, Definitions.ItemState.Visible);
        MainActivity.Companion.getDb().saveItem(currentItem);

        currentView.setIcon(new GroupDrawable(context, currentItem, 52));
    }

    private void deleteItem(Context context, final Item currentItem, Item dragOutItem, AppItemView currentView) {
        currentItem.getGroupItems().remove(dragOutItem);

        MainActivity.Companion.getDb().deleteItem(dragOutItem, false);
        MainActivity.Companion.getDb().saveItem(currentItem);
    ...
    public void updateItem(DesktopCallback callback, final Item currentItem, View currentView) {
        if (currentItem.getGroupItems().size() == 1) {
            final App app = MainActivity.Companion.getAppLoader().findItemApp(currentItem.getGroupItems().get(0));
            if (app != null) {
                Item item = MainActivity.Companion.getDb().getItem(currentItem.getGroupItems().get(0).getId());
                ...
                MainActivity.Companion.getDb().saveItem(item, MainActivity.Companion.getLauncher().getDesktop().getCurrentItem(), Definitions.ItemPosition.Desktop);
                MainActivity.Companion.getDb().saveItem(item, Definitions.ItemState.Visible);
                MainActivity.Companion.getDb().deleteItem(currentItem, false);

...
public final class ItemOptionView extends FrameLayout {
    ...
    protected void onDraw(@Nullable Canvas canvas) {
        ...
        if (canvas != null && _showFolderPreview && !_previewLocation.equals(-1.0f, -1.0f)) {
            ...
            canvas.drawCircle(_previewLocation.x, _previewLocation.y, ((float) Tool.dp2px((52 / 2) + 10)) * _folderPreviewScale, _paint);
        }
        if (_showFolderPreview) {
            invalidate();
        }
    ...
    public void showItemPopup(final MainActivity MainActivity) {
        ...
        float x = getDragLocation().x - MainActivity.Companion.getItemTouchX() + Tool.dp2px(10);
        float y = getDragLocation().y - MainActivity.Companion.getItemTouchY() - Tool.dp2px((46 * itemList.size()));

        if ((x + Tool.dp2px(200)) > getWidth()) {
            setPopupMenuShowDirection(false);
            x = getDragLocation().x - MainActivity.Companion.getItemTouchX() + MainActivity.getDesktop().getCurrentPage().getCellWidth() - Tool.dp2px(200) - Tool.dp2px(10);
        } else {
            setPopupMenuShowDirection(true);
        }

        if (y < 0) {
            y = getDragLocation().y - MainActivity.Companion.getItemTouchY() + MainActivity.getDesktop().getCurrentPage().getCellHeight() + Tool.dp2px(4);
        } else {
            y -= Tool.dp2px(4);
        }
        ...
        showPopupMenuForItem(x, y, itemList, new com.mikepenz.fastadapter.listeners.OnClickListener<PopupIconLabelItem>() {
            ...
            public boolean onClick(View v, IAdapter<PopupIconLabelItem> adapter, PopupIconLabelItem item, int position) {
                ...
                if (dragItem != null) {
//                    HpItemOption itemOption = new HpItemOption(MainActivity);
